...
class TodoDirective(SphinxDirective):
    ...
    def run(self):
        ...
        todo_node = todo('\n'.join(self.content))
        todo_node += nodes.title(locale._('Todo'), locale._('Todo'))
...
def process_todo_nodes(app, doctree, fromdocname):
    ...
    for node in doctree.findall(todolist):
        ...
        for todo_info in env.todo_all_todos:
            ...
            description = (
                locale._(
                    f"(The original entry is located in {filename}, line"
                    f" {todo_info['lineno']} and can be found ")
                )
            para += nodes.Text(description)

            # Create a reference
            newnode = nodes.reference('', '')
            innernode = nodes.emphasis(locale._('here'), locale._('here'))
